...
@export var journal_key: JournalConfig.Keys = JournalConfig.Keys.Wolf  # Default to Wolf
...
func take_hit(weapon_item_resource : WeaponItemResource) -> void:
	# Discover animal in journal when first hit
	print("[DEBUG] Animal: Taking hit, discovering in journal - key:", journal_key)
	EventSystem.JOU_discover.emit(int(journal_key))
